---
id: NOR-040
titulo: "Resolución n.º 327/2025, instrucciones atención diversidad 2025-2026"
nombre_corto: "Instrucciones Diversidad 25/26"
tipo_norma: "resolucion-administrativa"
ambito: "canarias"
autoridad: "DGOEII - Gobierno de Canarias"
fecha_disposicion: 2025-06-12
fecha_publicacion: 2025-06-18
fecha_entrada_vigor: 2025-09-01
fecha_consulta: 2026-04-26
fecha_analisis: 2026-04-26
estado_vigencia: "Superada"
fuente_principal: FTE-043
url_oficial: "https://www.gobiernodecanarias.org/educacion/web/servicios/atencion_diversidad/instrucciones-25-26/"
etapas_afectadas: [infantil, primaria, eso, bachillerato]
temas: [diversidad, instrucciones]
relaciones:
  desarrolla_a: [NOR-024]
  modificada_por: []
  modifica_a: []
  deroga_a: []
  derogada_por: []
  relacionada_con: []
nivel_evidencia: "confirmado-fuente-primaria"
---

# NOR-040 — Resolución n.º 327/2025, instrucciones diversidad 25/26

> Este resumen no sustituye la consulta de la fuente oficial.

## 1. Identificación

- **Norma:** Resolución n.º 327/2025.
- **Nombre corto:** Instrucciones Diversidad 25/26.
- **Ámbito:** Canarias.
- **Autoridad:** DGOEII - Gobierno de Canarias.
- **Fuente oficial:** `FTE-043`.
- **Fecha de consulta:** 2026-04-26.
- **Estado de vigencia:** Superada.

## 2. Objeto de la norma

Dicta las instrucciones para la organización y funcionamiento de la atención a la diversidad en los centros educativos de Canarias para el curso 2025-2026.

## 8. Resumen IA-friendly

Resolución administrativa que concreta la aplicación de las medidas de atención a la diversidad en los centros de Canarias durante el curso escolar 2025-2026.

## 11. Fuentes

- [FTE-043](../../../01_fuentes/portales-oficiales/FTE-043-resolucion-instrucciones-diversidad-25-26.md)
